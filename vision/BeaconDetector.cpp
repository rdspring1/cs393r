...
void BeaconDetector::detectBeacon(bool topCamera)
{
    ...
    int* position = 0;
    findBeacon(&vblocks_.world_object->objects_[WO_BEACON_PINK_YELLOW], pinkBlobs, yellowBlobs, topCamera, position);
    findBeacon(&vblocks_.world_object->objects_[WO_BEACON_YELLOW_PINK], yellowBlobs, pinkBlobs, topCamera, position);
    findBeacon(&vblocks_.world_object->objects_[WO_BEACON_BLUE_YELLOW], blueBlobs, yellowBlobs, topCamera, position);
    findBeacon(&vblocks_.world_object->objects_[WO_BEACON_YELLOW_BLUE], yellowBlobs, blueBlobs, topCamera, position);
    findBeacon(&vblocks_.world_object->objects_[WO_BEACON_PINK_BLUE], pinkBlobs, blueBlobs, topCamera, position);
    findBeacon(&vblocks_.world_object->objects_[WO_BEACON_BLUE_PINK], blueBlobs, pinkBlobs, topCamera, position);
}
...
bool BeaconDetector::findBeacon(WorldObject* beacon, BlobCollection& t, BlobCollection& b, bool topCamera, int* position)
{
    ...
    {   cout << "*** finding beacon" << endl;
        for(size_t tn = 0; tn < t.size(); ++tn)
        {
            for(size_t bn = 0; bn < b.size(); ++bn)
            {
                ...
                bCandidate.width = beacon->width;
                bCandidate.height = beacon->height;
                bCandidate.centerX = beacon->imageCenterX;
                bCandidate.centerY = beacon->imageCenterY;
                bCandidate.valid = true;
                bCandidate.fromTopCamera = topCamera;
                candidates[*position] = bCandidate;
                *position += 1;
                return true;
            }
        }
