  ...
  def run(self):
    ...
    if ball.seen:
      ...
      if math.fabs(diffX) < 45 and ball.fromTopCamera == False and ball.imageCenterY > 90:
        ...
        commands.setWalkVelocity(0.10, 0.0, moveAngle)
    else:
      previous_error = 0
      integralAngleError = 0
...
class WalkForwardNode(Node):
  def run(self):
    ...
    if not(ball.seen):
      ...
      if math.fabs(diffX) < 80 and math.fabs(diffX) > 35 and ball.fromTopCamera == False and ball.imageCenterY > 175 and math.fabs(goalBallError) < 100:
        previous_ball_angle = 0
        ball_distance = ball.visionDistance
        previous_ball_distance = ball_distance
        commands.stand()
